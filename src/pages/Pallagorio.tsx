...
const Pallagorio = () => {
  ...
  const tourContent = {
    en: {
      ...
      heroImage: "/lovable-uploads/a7a13b83-eb08-4c06-94da-a9b7574bfd65.png",
      highlights: [
        {
          title: "Vercino Caves",
  ...
  const { title, subtitle, intro, heroImage, highlights, details, whyChoose, bookNow, videoTitle, seoTags } = tourContent[language];
  ...
  return (
    ...
    >
      {/* Hero Section */}
      <section className="relative text-white py-16" style={{ backgroundImage: `url(${heroImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="font-serif text-3xl font-bold text-center mb-8">{videoTitle}</h2>
      ...
      <section className="py-12 bg-[#FDF8F6]">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto"><p className="text-lg leading-relaxed text-gray-700 mb-8">{intro}</p></div>
        </div>
      </section>
              ...
              <Card key={idx} className="hover:shadow-lg transition-shadow">
                {h.image && (
                  <div className="w-full h-48 overflow-hidden rounded-t-lg">
                    <OptimizedImage src={h.image} alt={h.title} className="w-full h-full object-cover" loading="lazy" />
                  </div>
                )}
                <CardHeader><CardTitle className="text-xl font-serif text-[#E2725B]">{h.title}</CardTitle></CardHeader>
                <CardContent><p className="text-gray-600 leading-relaxed">{h.description}</p></CardContent>
              </Card>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-[#FDF8F6]">
        <div className="container mx-auto px-4"><div className="max-w-4xl mx-auto">
            <h2 className="font-serif text-3xl font-bold mb-8 text-[#E2725B]">{language === "en" ? "Practical Details" : "Практическая информация"}</h2>
            <div className="grid md:grid-cols-2 gap-8">
              <Card><CardHeader><CardTitle className="text-xl">{language === "en" ? "Tour Format" : "Формат экскурсии"}</CardTitle></CardHeader><CardContent><p className="text-gray-600">{details.format}</p></CardContent></Card>
              <Card><CardHeader><CardTitle className="text-xl">{language === "en" ? "Includes" : "Включено"}</CardTitle></CardHeader><CardContent>
                <ul className="space-y-2">{details.includes.map((item,i)=><li key={i} className="flex items-start gap-2"><span className="text-[#E2725B] font-bold">•</span><span className="text-gray-600">{item}</span></li>)}</ul>
              </CardContent></Card>
            </div>
            <Card className="mt-8"><CardHeader><CardTitle className="text-xl">{language === "en" ? "Why Choose This Tour?" : "Почему выбрать эту экскурсию?"}</CardTitle></CardHeader><CardContent><div className="space-y-2">{whyChoose.map((r,i)=><p key={i} className="text-gray-600">{r}</p>)}</div></CardContent></Card>
        </div></div>
      </section>

      {/* SEO Tags Section */}
      <section className="py-8 bg-gray-50"><div className="container mx-auto px-4"><div className="max-w-4xl mx-auto text-center"><p className="text-sm text-gray-500 leading-relaxed">{seoTags}</p></div></div></section>

      {/* Booking Section */}
      <section className="py-16 bg-[#E2725B] text-white"><div className="container mx-auto px-4"><div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to="/tours"><Button size="lg" variant="outline" className="border-white text-white hover:bg-white hover:text-[#E2725B]">{language === "en" ? "View All Tours" : "Все экскурсии"}</Button></Link>
            </div>
        </div></div>
      </section>
    </Layout>


import React from "react";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Clock, Users, MapPin } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";
import { Link } from "react-router-dom";
import OptimizedImage from "@/components/ui/optimized-image";

interface ExcursionCardProps {
  id: string;
  title: string;
  description: string;
  price: number;
  duration: string;
  image: string;
  groupSize?: string;
  location?: string;
}

const ExcursionCard: React.FC<ExcursionCardProps> = ({
  id,
  title,
  description,
  price,
  duration,
  image,
  groupSize,
  location
}) => {
  const { language } = useLanguage();

  const handleBooking = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    
    if (language === "ru") {
      window.open("https://t.me/393446935576", "_blank");
    } else {
      window.open("https://wa.me/393446935576", "_blank");
    }
  };

  const formatPrice = (price: number) => {
    return language === "ru" ? `€${price}/чел` : `€${price}/person`;
  };

  const bookingText = language === "ru" ? "Забронировать" : "Book Now";

  return (
    <Link to={`/tours/${id}`} className="block group">
      <Card className="excursion-card overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-105 relative">
        {/* Image Container with Parallax Effect */}
        <div className="relative h-48 md:h-56 overflow-hidden">
          <OptimizedImage
            src={image}
            alt={`${title} - ${location || 'Calabria'} excursion`}
            className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
            loading="lazy"
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          
          {/* Price Badge */}
          <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-sm font-semibold text-gray-800">
            {formatPrice(price)}
          </div>
        </div>

        {/* Content */}
        <div className="card-content p-6 bg-gradient-to-br from-white to-gray-50">
          <h3 className="text-xl font-serif font-bold text-gray-800 mb-2 group-hover:text-[#0077B6] transition-colors duration-300">
            {title}
          </h3>
          
          <p className="text-gray-600 text-sm mb-4 line-clamp-3 leading-relaxed">
            {description}
          </p>

          {/* Details */}
          <div className="flex items-center gap-4 text-xs text-gray-500 mb-4">
            {duration && (
              <div className="flex items-center gap-1">
                <Clock className="w-3 h-3" />
                <span>{duration}</span>
              </div>
            )}
            {groupSize && (
              <div className="flex items-center gap-1">
                <Users className="w-3 h-3" />
                <span>{groupSize}</span>
              </div>
            )}
            {location && (
              <div className="flex items-center gap-1">
                <MapPin className="w-3 h-3" />
                <span>{location}</span>
              </div>
            )}
          </div>

          {/* Footer */}
          <div className="card-footer flex items-center justify-between">
            <span className="price text-lg font-bold text-[#0077B6]">
              {formatPrice(price)}
            </span>
            
            <Button 
              onClick={handleBooking}
              className="book-btn bg-gradient-to-r from-[#0077B6] to-[#00A9E6] hover:from-[#005A8A] hover:to-[#0077B6] text-white px-4 py-2 rounded-lg transition-all duration-300 transform hover:scale-105 flex items-center gap-2"
            >
              {language === "ru" ? (
                <>
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
                  </svg>
                  {bookingText}
                </>
              ) : (
                <>
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.525 3.687z"/>
                  </svg>
                  {bookingText}
                </>
              )}
            </Button>
          </div>
        </div>

        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#0077B6]/0 to-[#00A9E6]/0 group-hover:from-[#0077B6]/5 group-hover:to-[#00A9E6]/5 transition-all duration-300 pointer-events-none" />
      </Card>
    </Link>
  );
};

export default ExcursionCard;
